...
# 定义模型
class Net(nn.Module):
    def __init__(self, fc_cnt=3, neuron_cnt=800):
        super(Net, self).__init__()
        self.fc_cnt = max(2, fc_cnt)
        self.fc = nn.ModuleList()
        self.fc.append(nn.Linear(784, neuron_cnt))
        for i in range(fc_cnt-2):
            self.fc.append(nn.Linear(neuron_cnt, neuron_cnt))
            self.fc.append(nn.ReLU())
            self.fc.append(nn.Dropout(p=0.2))
        self.fc.append(nn.Linear(neuron_cnt, 10))

    def forward(self, x):
        x = x.view(-1, 784) # 调整x的形状（展平成一维向量）
        for fc in self.fc:
            x = fc(x)
