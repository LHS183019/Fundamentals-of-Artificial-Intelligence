...
public:
    ...
    void search(ValueEstimatorType value_of){
        ...
        // 用于记录进入优先队列的次序
        std::vector<StateType> queue_order;
        StateType state, new_state;
        
        states_queue.push(initial_state);
        queue_order.push_back(initial_state);
        ...
        while (not states_queue.empty()){
            ...
            for (ActionType action : state.action_space()){
                ...
                if (best_value_of.find(new_state) == best_value_of.end() 
                    ...
                    queue_order.push_back(new_state);
                    // TODO：记录路径——1行
        ...
        if (state.success()){
            std::cout << "Order of entering the priority queue: " << std::endl;
            for(auto& s:queue_order){
                if(typeid(s) == typeid(DirectedGraphState))
                {
                    std::cout << dynamic_cast<DirectedGraphState*>(&s)->current_node() << " ";
                }else{
                    s.show();
                }
            }
            std::cout << std::endl;
            std::cout << "Output solution path: ";
            show_reversed_path(last_state_of, state);
            std::cout << "Cost of the solution path: " << state.cumulative_cost() << std::endl;
        }
