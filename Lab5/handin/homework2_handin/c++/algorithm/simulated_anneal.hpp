...
private:
    ...
    const StateLocalType& sample_path(ValueEstimatorType value_of, TemperatureScheduleType temperature_at){
        ...
        // 终止条件：存在邻居状态+温度条件
        int t=0;
        for (t = 0; state.neighbor_count() > 0 and temperature >= terminate_temperature; ++ t){
            ...
            if (value_diff > 0 or RandomVariables::uniform_real() < std::exp(value_diff / temperature))
            {
                state = new_state;
            }
        }
        
        // std::cout << "temperature of"<<t<<":" << temperature_at(t) << std::endl;
        return state;
    }
