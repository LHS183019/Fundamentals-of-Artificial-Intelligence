...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.act4(self.cnn4(x))
        return x.squeeze().squeeze().squeeze() # 移除大小为1的维度，(1,1,1) -> () 即x变成标量
...
def train():
    ...
    for epoch in range(flags["n_epoch"]):
        ...
        for i, (real_images, _) in enumerate(train_loader):
            ...
            #  真实图片通过判别器D，得到真实图片的判别结果，记为D(x)
            D_x = D.forward(real_images)
            #  生成图片通过判别器D，得到生成图片的判别结果，记为D(G(z))
            random_z = torch.randn(batch_size, flags["z_dim"], device=device)
            fake_images = G.forward(random_z)
            D_G_z = D.forward(fake_images.detach())
            #  计算判别器D的损失，使其能够区分真实图片和生成图片：loss = -log(D(x)) - log(1-D(G(z)))
            d_loss_real = criterion(D_x, real_labels)
            d_loss_fake = criterion(D_G_z, fake_labels)
            ...
            #  计算生成器G的损失，使其能够生成更逼真的图片：loss = -log(D(G(z)))
            D_G_z = D.forward(fake_images)
            g_loss = criterion(D_G_z, real_labels)
            #  计算生成器G的梯度，更新生成器G的参数
            g_loss.backward()
            g_optimizer.step()
